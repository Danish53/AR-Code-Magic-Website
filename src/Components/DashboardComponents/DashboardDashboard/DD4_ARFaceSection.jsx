...
import { generateArFace } from "../../../redux/arTextSlice";
...
const DD4_ARFaceSection = () => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      data.append("arImage", image);
      data.append("user_id", user_id);
      data.append("ar_type", "AR Face");

      const result = await dispatch(generateArFace({ formData: data })).unwrap();

      if (result.data?.qr_code) setQrCodeUrl(result.data.qr_code);

      toast.success("AR Face generated successfully!");
    } catch (err) {
      console.error("❌ AR Face Generation Error:", err);
      toast.error("Failed to generate AR Face");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="container mt-5">
      <h4>Upload your logo for AR Face filter</h4>
      <input
        type="file"
        accept="image/png, image/jpeg"
        onChange={handleImageUpload}
      ...
      <button
        ...
        disabled={loading}
      >
        {loading ? "Processing..." : "Submit Logo"}
      </button>
      ...
      {qrCodeUrl && (
        <div className="mt-4 text-center">
          <h6>Scan this QR to open your AR Face:</h6>
          <img
            src={qrCodeUrl}
            alt="QR Code"
